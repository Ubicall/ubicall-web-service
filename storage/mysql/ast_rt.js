...
function init(_settings) {
    return when.promise(function(resolve, reject) {
        ...
        _sequelize.authenticate().then(function() {
            log.info("connected successfully to DB => %s:%s:%s", settings.storage.ast_rt_mysql.database, _host, _port);
        }).catch(function(error) {
            log.error("Unable to connect to DB => %s:%s:%s", settings.storage.ast_rt_mysql.database, _host, _port);
            throw error;
        });
