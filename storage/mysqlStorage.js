...
return when.promise(function(resolve,reject){
$client.findAll({where:{license_key:key}}).then(function(client){
...
function getVersion(key)
{
  ...
  //Get Clients with a specific license_key
    $client.findOne({ where: {  license_key: key}  }).then(function(client){

      //Now I have client attributes with a specific license_key

      return $version.findOne({ where:{client_id:client.id } }).then(function(version){
  ...
  function getAccountInfo(key){
    return when.promise(function(resolve,reject){
      ...
      $company.findAll({where:{license_key:key}}).then(function(company){
